...
These two repositories are best understood as one project in two forms: a scaffolded version and a fully implemented browser game. Both are built as plain front-end applications using `HTML`, `CSS`, and vanilla `JavaScript`, with no build step and no backend service. The core idea is the same in both: a timed word game where players build words from a constrained set of letters and score points based on valid submissions.

The `Word-Blast-Word-Duel-Prototype` repository preserves the structural plan for the game. The `Word-Blast-Interactive-Word-Duel` repository contains the live runtime: working game state, timers, DOM updates, API-backed validation, and AI turns. Together they show both the designed architecture and the implemented version of the same game loop.
...
So it's more than a one-screen word checker — it includes a full game cycle with turn sequencing, intermission state, and match results.
...
Even if the exact development timeline wasn't a strict prototype-then-build sequence, the two repositories still serve as a clean split between planned structure and implemented runtime.
...
This gives it a clear client-side state model:

- player-level persistent data
- session-level turn data
- game-level orchestration

For a vanilla JavaScript browser game, it's pretty well structured. Instead of collapsing everything into a single procedural script, it uses explicit runtime objects to model the game loop.

![Game Loop Flow](./assets/diagrams/game-loop-flow.png)

## Input Constraints and Turn Logic

Each session generates a 10-letter lineup and guarantees that at least three of those letters are vowels. The player can only use letters that actually appear in that lineup, and duplicate letters can only be used as many times as they're present in the rack.

The input handling enforces this in real time:
...
So the rack is part of both the scoring rules and the live input constraint system, not just a static display.

The game updates the score and the current word list immediately after each submission, and keeps valid and invalid words separated by visual state. Score and timer colors shift based on game conditions — visual feedback is tied directly to the game state.
...
So the game has a runtime validation path based on an external dictionary source, not just a local word list or hard-coded rule set. It also means the browser version isn't purely self-contained — it includes live network-dependent validation as part of the play loop.

## AI Mode and Streaming JSON Assembly

The most interesting technical feature in the interactive version is the AI mode.

In AI turns, the game calls the OpenAI chat completions API directly from the browser and asks it to generate candidate words from the current letter rack. The response is requested as a streamed function-call payload rather than a single final response. The script manually processes the streamed chunks, extracts the incremental function-call arguments, and feeds them into a custom `JsonAssembler`.

The `JsonAssembler` is a separate utility that incrementally reconstructs structured JSON from streamed text fragments. Instead of waiting for a full JSON string, it parses the incoming text as it arrives and emits partial updates as the `words` array becomes available.

That parsed word stream is then pushed into a timed submission queue. AI words are fed back through the same input and submission flow used by a human player, so the AI turn behaves like an actual in-game participant rather than a scoring shortcut.

The integration covers a lot of ground for a browser game:
...
The UI behavior is driven directly from DOM manipulation and event listeners in the script, no framework involved. It ends up being a good showcase of how far you can take vanilla JavaScript with non-trivial runtime state.
