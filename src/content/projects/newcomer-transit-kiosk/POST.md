...
This project is a touch-oriented React kiosk application designed to present transit, orientation, and emergency information for someone arriving in Calgary. It's organized as a single-screen kiosk experience, not a browser-style website. Instead of URL routing, the interface is driven by a single page state in `App.js`, and each major screen is swapped in place as the user moves through the kiosk. Simple and predictable navigation for a public-facing information terminal.

The home screen acts as the central entry point into three main areas: transit information, points of interest, and emergency contact support. A shared header and persistent taskbar keep those screens connected, with the same core actions available throughout — return to home, language switching, and QR output when a page has something that can be handed off to a phone.

## Navigation and Screen Structure

The application uses a state-based screen controller instead of `react-router`. Each page component receives `setPage`, and moving between screens is handled through explicit page names like `Home`, `Transit`, `PointsOfInterest`, `Language`, `Emergency`, `TransitRoutes`, `FareInformation`, and `PlanATrip`. The application always knows which full-screen view is active, and each screen handles one focused task.

The `Header` component provides a consistent title and back action for a stable "return" path across nested screens. The `Taskbar` sits at the bottom and acts as a shared utility strip — when a page can generate a useful external destination, the taskbar switches from a disabled QR placeholder to an active QR code that can be scanned from a mobile device.
...
`PlanATrip` is the most interactive part of the transit section. It uses Google Maps Places and Directions services to search destinations, select locations from search results or directly from the map, switch between walking, transit, and driving modes, and render a route on the embedded map. The directions panel walks the nested Google directions structure recursively, so transit segments and sub-steps can be rendered as a readable step-by-step list inside the kiosk UI.

![Feature architecture](./assets/diagrams/feature-architecture.png)

## Points of Interest and Emergency Tools

The `PointsOfInterest` page takes a different approach. Instead of asking Google for a general text search, it uses a built-in catalog of Calgary destinations with names, coordinates, category labels, distances, and descriptions — a curated, city-guide approach rather than an open-ended search box. Users can filter by category, narrow by distance with a slider, select a destination from the filtered list, and request route directions. Once a location is selected, the page switches from browsing mode into route-preview mode and can generate a QR code that opens the destination in Google Maps on a phone.

It's a combination of a fixed local dataset with live route rendering — part directory, part guided recommendation layer on top of a map service.

The emergency area is split into a contact page and a request workflow. The main emergency screen gives quick access to emergency-related information, while `HelpIsOnTheWay` simulates a simplified assistance request flow. The user selects one or more response categories (ambulance, police, or firefighters), submits or updates the request, and sees a timed progress state that transitions from "submitting" into "help is on the way." The implementation is entirely front-end driven, but it still models the state changes of a guided request process rather than just showing a static list of phone numbers.

## Language Support and Mobile Handoff

Language switching is a visible part of the kiosk design, not hidden away in settings. The language screen presents a grid of options and routes the selection through `react-i18next`. Localization is available application-wide from startup, and the selection flow returns the user to the home screen after confirmation.

The language list is larger than the currently active translation set. The screen shows a broad set of language names, but the implementation supports English and Korean as the working options while routing unsupported choices into a modal message — part navigation surface, part roadmap for future internationalization.

The QR behavior in the taskbar is another recurring system. Rather than embedding full mobile workflows directly into the kiosk, pages can hand off a destination or purchase link by rendering a QR code in the persistent bottom bar. This pattern shows up in route planning, points of interest, and fare information — a consistent way to bridge from a shared public screen to a personal device.
