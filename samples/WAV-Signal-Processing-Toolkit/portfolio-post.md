...
This project combines several concrete concerns in one compact codebase:

- binary WAV file parsing
- PCM metadata validation
- mono and stereo channel handling
- normalized sample-vector processing
- direct time-domain convolution
- FFT-based frequency-domain convolution
- output amplitude scaling
- WAV file regeneration
- structural and sample-level regression checking

Taken together, the repository shows both systems-level file handling and signal-processing logic. The defining characteristic is the pairing of a custom WAV I/O layer with two different convolution implementations and a validation utility that checks the resulting audio against a reference file.

## Tags

- Project Size: Medium
- Technical Skill Level: High
- Technology Tags: C++, WAV/PCM, CLI
- Capability Tags: Signal Processing, FFT, File I/O, Algorithms, Binary Parsing, Regression Checking
